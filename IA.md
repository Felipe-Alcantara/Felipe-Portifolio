...
[2026-06-01] **Sync no deploy** — O site NÃO lê o GitHub em tempo real; READMEs/metadados são arquivos estáticos em `src/data/github-import/` empacotados no build. Por isso o deploy (`deploy.yml`) roda `node scripts/sync-github-repos.mjs` antes do `npm run build`, garantindo conteúdo atual a cada publicação. Token via Secret `GH_SYNC_TOKEN` (escopo `repo`, inclui privados); sem ele só públicos. `GITHUB_USERNAME` fixado no workflow. Após o sync, o workflow COMMITA os dados de volta na `main` (`chore(data): ... [skip ci]`, com `permissions: contents: write`) para que repositório local e produção não divirjam — por isso o job usa `[skip ci]`, evitando loop de deploy. No CI o sync usa `node scripts/...` com `env:` em vez do script `sync:github` do package.json, porque este depende de `--env-file=.env`, inexistente no CI.  
[2026-06-01] **Curadoria opt-out** — Repos novos descobertos pelo sync APARECEM automaticamente no site (decisão do Felipe). Para esconder, há dois mecanismos: `hidden: true` no override (respeitado pelo site no merge `{...generated, ...override}` + filtro `!p.hidden` em [projects.jsx](src/data/projects.jsx)) ou `repoKey` no ignore.  
[2026-06-01] **Remoção permanente de projeto = `portfolio-items.ignore.json`** — Único mecanismo que o SYNC respeita: repos com `repoKey` na lista nem são baixados ([sync-service.js](src/utils/github-import/sync-service.js) filtra antes do fetch e descarta no merge). Apagar item só do `generated.json` faz ele REAPARECER no próximo sync. Hoje no ignore: `desktop-tutorial`, `trabalho-da-faculdade`, `felixoverse-official-bot`.  
[2026-06-01] **Pegadinha do rename** — Tanto `ignore.json` quanto `hidden` (override) amarram por `repoKey` = `owner/nome-do-repo`. Renomear o repo no GitHub MUDA o repoKey, então a curadoria amarrada ao nome antigo para de valer e o repo reaparece com o nome novo. Caso real: `felixoverse-official-bot` (privado, renomeado) reapareceu quando o sync rodou no CI com token de privados. Ao renomear um repo curado, atualizar o repoKey no ignore/override.  
